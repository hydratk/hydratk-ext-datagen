...
class ASN1Codec(object):
    ...
    def __init__(self):
        ...
        if (not (version_info[0] == 2 and version_info[1] == 7)):
            raise NotImplementedError(
                'ASN.1 codec is not supported for Python 2.6 and 3.x due to external library libmich')
    ...
    def import_spec(self, filename):
        ...
        try:
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_import_spec', filename), self._mh.fromhere())
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_spec_imported'), self._mh.fromhere())
    ...
    def encode(self, infile, element, outfile=None):
        ...
        try:
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_encode', infile), self._mh.fromhere())
            ...
            if (ev.will_run_default()):
                ...
                if (path.exists(infile)):
                    ...
                    outfile = path.abspath(infile).split(
                        '.')[0] + '.bin' if (outfile == None) else outfile
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_encoded', outfile), self._mh.fromhere())
    ...
    def decode(self, infile, element, outfile=None):
        ...
        try:
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_decode', infile), self._mh.fromhere())
            ...
            if (ev.will_run_default()):
                ...
                if (path.exists(infile)):
                    ...
                    outfile = path.abspath(infile).split(
                        '.')[0] + '.json' if (outfile == None) else outfile
                    with open(outfile, 'w') as f:
                        ...
                        if (len(records) > 1):
                            ...
                            for record in records:
                                ...
                                output.append(
                                    self._create_dict(self._elements[element]))
                        else:
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg(
                'datagen_asn1_decoded', outfile), self._mh.fromhere())
        ...
        if (obj._type in ('SEQUENCE', 'SET')):
            for key in obj._cont._index:
                if (key in val):
                    ...
                    output[key] = self._create_dict(
                        obj._cont._dict[key], val[key], output[key])
            ...
            output[key] = OrderedDict()
            output[key] = self._create_dict(
                obj._cont._dict[key], val[1], output[key])
        else:
