...
class Adapter(object):
    ...
    def parse_test_suite(self, suite, outfile=None):
        ...
        try:
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('datagen_adapter_parsing_suite', suite), self._mh.fromhere()) 
            ...
            if (ev.will_run_default()):  
                ...
                self._suite = {'file': suite, 'title': rows[0].xpath('b')[0].text}
        
                self._tests = []
                for row in rows[1:]:
                    elem = row.xpath('a')[0]
                    test = {'title': elem.text, 'file': elem.attrib['href']}
                    test['url'], test['steps'] = self.parse_test(path.join(path.dirname(suite), test['file']))
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('datagen_adapter_suite_parsed'), self._mh.fromhere())   
    ...
    def parse_test(self, test):
        ...
        try:
            ...
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('datagen_adapter_parsing_test', test), self._mh.fromhere()) 
            ...
            if (ev.will_run_default()):  
                ...
                for row in rows:
                    cols = row.xpath('td')
                    step = {'command': cols[0].text, 'target': cols[1].text, 'value': cols[2].text}
                    steps.append(step)
                    
            self._mh.dmsg('htk_on_debug_info', self._mh._trn.msg('datagen_adapter_test_parsed'), self._mh.fromhere())   
    ...
    def handle_command(self, step, url):
        ...
        handler = cfg.mapping[step['command']] if (step['command'] in cfg.mapping) else None
        if (handler == None):
            print(self._mh._trn.msg('datagen_adapter_cmd_unknown', step['command']))
            test, validate = """print('Unknown command: {0}')""".format(step['command']), """assert False"""
        elif (handler == 'cmd_dummy'):
            print(self._mh._trn.msg('datagen_adapter_cmd_dummy', step['command']))
            test, validate = """print('Not supported command: {0}')""".format(step['command']), """assert False"""
        else:
    ...
    def cmd_alert(self, step, *args):
        ...
        if (command in ['assertAlert', 'assertConfirmation', 'assertNotAlert', 'assertNotConfirmation',
            ...
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'alert = {1}'""".format(step['target'], step['target'])
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'alert != {1}'""".format(step['target'], step['target'])
            ...
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (not res), 'alert present'"""
            else:
                validate = cfg.tmpl_validate + """assert (res), 'alert not present'"""
        ...
        res = c.confirm_alert"""
                validate = cfg.tmpl_validate + """assert (res), 'confirm_alert != True'"""
            else:
                test = """c.confirm_alert = False
        res = c.confirm_alert"""
                validate = cfg.tmpl_validate + """assert (not res), 'confirm_alert != False'"""                      
    ...
    def cmd_go_back(self, step, *args):
        ...
        test = """res = c.go_back()"""
        validate = cfg.tmpl_validate + """assert (res), 'previous page not displayed'"""
    ...
    def cmd_open(self, step, url, *args):
        ...
        validate = cfg.tmpl_validate + """assert (res), 'page {0} not opened'""".format(url+step['target'])
    ...
    def cmd_set(self, step, *args):
        ...
        if (command in ['sendKeys', 'sendKeysAndWait', 'type', 'typeAndWait']):
            test = """res = c.set_element('{0}', '{1}', method='{2}')""".format(ident, value, method)
        elif (command in ['check', 'checkAndWait', 'uncheck', 'uncheckAndWait']): 
            test = """res = c.set_element('{0}', method='{1}', el_type='checkbox')""".format(ident, method)
        elif (command in ['click', 'clickAndWait', 'submit', 'submitAndWait']): 
            test = """res = c.set_element('{0}', method='{1}', el_type='submit')""".format(ident, method)            
        elif (command in ['select', 'selectAndWait']):
            test = """res = c.set_element('{0}', '{1}', method='{2}', el_type='select')""".format(ident, value, method)
        
        validate = cfg.tmpl_validate + """assert (res), 'element {0} not set'""".format(ident)
    ...
    def cmd_store(self, step, *args):
        ...
        my.pocket.content['{2}'] = res""".format(ident, method, value)
            validate = cfg.tmpl_validate + """assert (res != None), 'element not stored'"""  
        elif (command in ['storeElementPresent']):
            test = """res = c.get_element('{0}', '{1}')
        my.pocket.content['{2}'] = res != None""".format(ident, method, value)
            validate = cfg.tmpl_validate + """assert (res != None), 'element presence not stored'"""  
    ...
    def cmd_verify(self, step, *args):
        ...
        if (command in ['assertNotText', 'assertText', 'verifyNotText', 'verifyText']):
            test = """res = c.read_element('{0}', '{1}')""".format(ident, method)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'element {1} = {2}'""".format(value, ident, value)
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'element {1} != {2}'""".format(value, ident, value)
        elif (command in ['assertElementPresent', 'assertElementNotPresent', 'verifyElementNotPresent', 'verifyElementPresent']):
            test = """res = c.get_element('{0}', '{1}')""".format(ident, method)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (res == None), 'element {0} present'""".format(ident)
            else:
                validate = cfg.tmpl_validate + """assert (res != None), 'element {0} not present'""".format(ident)
        elif (command in ['assertAttribute', 'assertNotAttribute', 'verifyAttribute', 'verifyNotAttribute']):
            target = ident.split('@')
            ident, attr = target[0], target[1]
            test = """res = c.get_element('{0}', '{1}').get_attribute('{2}')""".format(ident, method, attr)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'element {1} attribute {2} = {3}'""".format(value, ident, attr, value)
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'element {1} attribute {2} != {3}'""".format(value, ident, attr, value)
        elif (command in ['assertBodyText', 'assertNotBodyText', 'verifyBodyText', 'verifyNotBodyText']):
            test = """res = c.read_element('BODY', 'tag')""" 
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'body = {1}'""".format(target, target) 
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'body != {1}'""".format(target, target)  
        elif (command in ['assertChecked', 'assertNotChecked', 'assertNotSelected',
                          'verifyChecked', 'verifyNotChecked']):
            test = """res = c.get_element('{0}', '{1}').is_selected()""".format(ident, method)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (not res), 'element {0} selected'""".format(ident)
            else:
                validate = cfg.tmpl_validate + """assert (res), 'element {0} not selected'""".format(ident)
        elif (command in ['assertLocation', 'assertNotLocation', 'verifyLocation', 'verifyNotLocation']):  
            test = """res = c.get_current_url()"""
            if ('Not' in step['command']):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'URL = {1}'""".format(target, target)
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'URL != {1}'""".format(target, target) 
        elif (command in ['assertNotTitle', 'assertTitle', 'verifyNotTitle', 'verifyTitle']):  
            test = """res = c.get_title()"""
            if ('Not' in step['command']):
                validate = cfg.tmpl_validate + """assert (res != '{0}'), 'title = {1}'""".format(target, target)
            else:
                validate = cfg.tmpl_validate + """assert (res == '{0}'), 'title != {1}'""".format(target, target)    
        elif (command in ['assertNotValue', 'assertValue', 'verifyNotValue', 'verifyValue']):
            test = """res = c.get_element('{0}', '{1}').get_attribute('value')""".format(ident, method)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (not res), 'element {0} checked'""".format(ident)
            else:
                validate = cfg.tmpl_validate + """assert (res), 'element {0} not checked'""".format(ident) 
        elif (command in ['assertNotVisible', 'assertVisible', 'verifyNotVisible', 'verifyVisible']):
            test = """res = c.get_element('{0}', '{1}').is_displayed()""".format(ident, method)
            if ('Not' in command):
                validate = cfg.tmpl_validate + """assert (not res), 'element {0} visible'""".format(ident)
            else:
                validate = cfg.tmpl_validate + """assert (res), 'element {0} not visible'""".format(ident)  
        elif (command in ['assertNotXpathCount', 'assertXpathCount', 'verifyNotXpathCount', 'verifyXpathCount']):
            test = """res = c.get_element('{0}', 'xpath', False)""".format(ident)
            if ('Not' in command):
                validate = """this.test_result = len(res) if (res != None) else 0 
    ...
    def cmd_wait(self, step, *args):
        ...
        if (command in ['waitForElementNotPresent', 'waitForElementPresent']):
            if ('Not' in command):
                cond = """c.get_element('{0}', '{1}') == None""".format(ident, method)
            else:
                cond = """c.get_element('{0}', '{1}') != None""".format(ident, method)
        elif (command in ['waitForNotText', 'waitForText']):
            if ('Not' in command):
                cond = """c.read_element('{0}', '{1}') != '{2}'""".format(ident, method, value)
            else:
                cond = """c.read_element('{0}', '{1}') == '{2}'""".format(ident, method, value)
            ...
            ident, attr = target[0], target[1] 
            if ('Not' in command):
                cond = """c.get_element('{0}', '{1}').get_attribute('{2}') != '{3}'""".format(ident, method, attr, value)
            else:
                cond = """c.get_element('{0}', '{1}').get_attribute('{2}') == '{3}'""".format(ident, method, attr, value)
        elif (command in ['waitForBodyText', 'waitForNotBodyText']):
            if ('Not' in command):
                cond = """c.read_element('BODY', 'tag') != '{0}'""".format(target)
            else:  
                cond = """c.read_element('BODY', 'tag') == '{0}'""".format(target) 
        elif (command in ['waitForChecked', 'waitForNotChecked']):
            if ('Not' in command):
                cond = """c.get_element('{0}', '{1}').is_selected() == False""".format(ident, method)
            else:  
                cond = """c.get_element('{0}', '{1}').is_selected() == True""".format(ident, method)      
            ...
            if ('Not' in command):
                cond = """c.get_element('{0}', '{1}').get_attribute('value') == False""".format(ident, method)
            else: 
                cond = """c.get_element('{0}', '{1}').get_attribute('value') == True""".format(ident, method)
        elif (command in ['waitForNotVisible', 'waitForVisible']):
            if ('Not' in command):
                cond = """c.get_element('{0}', '{1}').is_displayed() == False""".format(ident, method)
            else:    
                cond = """c.get_element('{0}', '{1}').is_displayed() == True""".format(ident, method)  
        elif (command in ['waitForNotXpathCount', 'waitForXpathCount']):
            if ('Not' in command):
                cond = """len(c.get_element('{0}', 'xpath', False)) != {1}""".format(ident, value) 
            else: 
                cond = """len(c.get_element('{0}', 'xpath', False)) == {1}""".format(ident, value)          
